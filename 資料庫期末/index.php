...
<?php 
    ...
    $yeardate=date("d");
    //月花費
    $user_sql = "SELECT `eMoney` FROM expenses AS E NATURAL JOIN record AS R  WHERE R.mid=$mid AND E.rid=R.rid AND `date` LIKE '$yeartime%'";
    ...
    //總花費
    $totalcost = "SELECT `eMoney` FROM expenses AS E NATURAL JOIN record AS R  WHERE R.mid=$mid AND E.rid=R.rid ";
    ...
    //月收入
    $earn = "SELECT `iMoney` FROM income AS I NATURAL JOIN record AS R  WHERE R.mid=$mid AND I.rid=R.rid AND `date` LIKE '$yeartime%'";
    ...
    //總收入
    $totalearn = "SELECT `iMoney` FROM income AS I NATURAL JOIN record AS R  WHERE R.mid=$mid AND I.rid=R.rid ";
    ...
    //算月收支總收支比例
    $monthEC=0;
    $monthEC=$MonthTotalEarn+$MonthTotal;
    $totalEC=0;
    $totalEC=$aTotalEarn+$TotalExp;
    $meanmonthEC=0;
    $meantotalEC=0;
    if($MonthTotal!=0&&$MonthTotalEarn!=0){
       $meanmonthEC=round(($MonthTotal*-1)/$MonthTotalEarn*100,2); 
    }
    else{
        $meanmonthEC=0;
    }
    if($TotalExp!=0&&$aTotalEarn!=0){
        $meantotalEC=round(($TotalExp*-1)/$aTotalEarn*100,2);
    }
     else{
         $meantotalEC=0;
     }
    //每個月初結算
    $exuser_sql = "SELECT `eMoney` FROM expenses AS E NATURAL JOIN record AS R  WHERE R.mid=$mid AND E.rid=R.rid AND `date` LIKE '$yeartime2%'";
    $exuser_result = mysqli_query($db_link, $exuser_sql);
    $exMonthTotal=0;
    if($exuser_result) {
        while ($exrow_result = mysqli_fetch_assoc($exuser_result)) {                                               
                $exMonthTotal=$exMonthTotal+$exrow_result["eMoney"];
        }
    }
    $exearn = "SELECT `iMoney` FROM income AS I NATURAL JOIN record AS R  WHERE R.mid=$mid AND I.rid=R.rid AND `date` LIKE '$yeartime2%'";
    $exMonthEarn = mysqli_query($db_link, $exearn);
    $exMonthTotalEarn=0;

    if($exearn) {
        while ($exMonthEarned = mysqli_fetch_assoc($exMonthEarn)) {                                               
                $exMonthTotalEarn=$exMonthTotalEarn+$exMonthEarned["iMoney"];
        }
    }
    $exmonthEC=0;
    $exmonthEC=$exMonthTotalEarn+$exMonthTotal;
    $meanexmonthEC=0;

    if($exMonthTotal!=0&&$exMonthTotalEarn!=0){
        $meanexmonthEC=round(($exMonthTotal*-1)/$exMonthTotalEarn*100,2); 
     }
     else{
         $meanexmonthEC=0;
     }
    $ifhave = "SELECT EImonth FROM book  WHERE `EImonth` LIKE '$yeartime2%' AND `mid`=$mid ";
    ...
    if($yeardate==17&&$user_ifhave[0]==null){
        ...
        $monthcount = "INSERT INTO book (`bid`,`ExIn`,`per`,`EImonth`,`mid`) VALUES (null,'$exmonthEC','$meanexmonthEC','$yeartime2','$mid')";
        $monthcount_result=mysqli_query($db_link, $monthcount);
    }
    //圓餅圖資料
    $expencepie = "SELECT * FROM (SELECT * FROM record WHERE mid=$mid)AS R,expenses AS P WHERE R.rid=P.rid AND `date` LIKE '$yeartime%'";
    ...
    //印出前幾個月收支比例
    $monthEItable = "SELECT * FROM book WHERE mid=$mid ";
                ...
                <div class="container-fluid">
                                    ...
                                    <div class="row no-gutters align-items-center">
                                        <div class="col mr-2">
                                            <div class="text-xs font-weight-bold text-success text-uppercase mb-1">
                                                總支出 </div>
                                            <div class="h5 mb-0 font-weight-bold text-gray-800"><?php echo$TotalExp?></div>
                                        </div>
                                        <div class="col-auto">
                        ...
                        <div class="col-xl-3 col-md-6 mb-4">
                            <div class="card border-left-info shadow h-100 py-2">
                                <div class="card-body">
                                    <div class="row no-gutters align-items-center">
                                        <div class="col mr-2">
                                            <div class="text-xs font-weight-bold text-info text-uppercase mb-1"><?php echo$Current?>月收入
                                            </div>
                                            <div class="row no-gutters align-items-center">
                                                <div class="col-auto">
                                    ...
                                    <div class="row no-gutters align-items-center">
                                        <div class="col mr-2">
                                            <div class="text-xs font-weight-bold text-warning text-uppercase mb-1">
                                                總收入</div>
                                            <div class="h5 mb-0 font-weight-bold text-gray-800"><?php echo$aTotalEarn?></div>
                                        </div>
                                        <div class="col-auto">
                        ...
                        <div class="col-xl-8 col-lg-7">
                            <div class="card shadow mb-4">
                                <div
                                    ...
                                    <h6 class="m-0 font-weight-bold text-primary">支出表</h6>
                                    ...
                                    <thead>
                                        ...
                                        if($monthSItable) {
                                            while ($monthSItable_result = mysqli_fetch_assoc($monthSItable)) {                                               
                                                echo " <tr><td>".$monthSItable_result["EImonth"]."</td>";
                                                echo "<td>".$monthSItable_result["ExIn"]."</td>";
                                                echo "<td>".$monthSItable_result["per"]."</td></tr>";                                                 
                                            }
                                        }
                                        ?>
                                </table>
                                ...
                                <div class="card-body">
                                    <div class="row no-gutters align-items-center">
                                        <div class="col mr-2">
                                            <div class="text-xs font-weight-bold text-success text-uppercase mb-1">
                                                單月支出佔收入比 </div>
                                            <div class="h5 mb-0 font-weight-bold text-gray-800"><?php echo$meanmonthEC?>%</div>
                                        </div>
                                        <div class="col-auto">
                                            <i class="fas fa-dollar-sign fa-2x text-gray-300"></i>
                                        </div>                                        
                                ...
                                <div class="card-body">
                                    <div class="row no-gutters align-items-center">
                                        <div class="col mr-2">
                                            <div class="text-xs font-weight-bold text-success text-uppercase mb-1">
                                                總支出佔收入比 </div>
                                            <div class="h5 mb-0 font-weight-bold text-gray-800"><?php echo$meantotalEC?>%</div>
                                        </div>
                                        <div class="col-auto">
                                            <i class="fas fa-dollar-sign fa-2x text-gray-300"></i>
                                        </div>
                    ...
                    <div class="row">
                        ...
                        <div class="col-lg-12 mb-5">
                            ...
                            <div class="card shadow mb-4">
                                <div class="card-header py-3">
                                    <h6 class="m-0 font-weight-bold text-primary">消費組成</h6>
                                </div>
                                <div class="card-body center">
                                <div id="piechart" style="width: 50rem; height: 30rem;"></div>
                                </div>
                            </div>
                        ...
                        <div class="card shadow mb-4">
                            ...
                            <div class="card-body center">
                            <img src="graph.php?mid=<?php echo$mid?>&accnum=<?php echo$accnum?>&password=<?php echo$password?>&mname=<?php echo$mname?>" />
                            </div>
                        </div>
                        <div class="card shadow mb-4">
                            <div class="card-header py-3">
                            ...
                            <div class="card-body center">
                            <img src="linegraph.php?mid=<?php echo$mid?>&accnum=<?php echo$accnum?>&password=<?php echo$password?>&mname=<?php echo$mname?>" />
                            </div>
                        </div>
